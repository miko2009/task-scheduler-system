...
def create_or_rotate(app_user_id: str, device_id: str, platform: str, app_version: str, os_version: str, expires_at: str) -> tuple[str, str]:
    conn = None
    ...
    try:
        ...
        if not session:
            ...
            cursor.execute("""
                UPDATE app_sessions
                SET token_hash = %s, token_encrypted = %s, expires_at = %s, updated_at = %s
                WHERE session_id = %s
            """, (token_hash, token_encrypted, expires_at, now, session_id))
        conn.commit()
